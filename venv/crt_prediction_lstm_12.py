...
# Feature set:
#   (1) execution time vector (absolute, accumulated)
#   (2) sequence of event
...
# calculate execution time of each event
...
for i in range(num_of_traces):
    ...
    for j in range(num_of_events_in_trace):
        ...
        exe_time = (cur_time - prev_time).total_seconds() / 60
        exe_time_lst.append(exe_time)
        event_idx += 1


df_exe_time = pd.DataFrame(exe_time_lst)
...
preprocess = make_column_transformer(
    ...
    (RobustScaler(), ['seq_of_event', 'time_from_trace_start', 'num_of_events_hour_of_day',
     'num_of_events_day_of_week', 'case_remaining_time'])
)
...
scaler = RobustScaler()
exe_time_arr = scaler.fit_transform(df_exe_time)
