...
# Method: LSTM (seq_len = each trace length, batch_size = each trace length, input_size = 24)
# Feature set:
...
# dtype = torch.cuda.FloatTensor # for cuda
...
class LSTM(nn.Module):
    ...
    def init_hidden(self):
        ...
        # add ".type(dtype)" to each argument if you want to use cuda
        return (torch.zeros(self.num_layers, self.batch_size, self.hidden_dim),
                torch.zeros(self.num_layers, self.batch_size, self.hidden_dim))
    def forward(self, input):
        # transform (batch -> packed_padded_batch)
        #  (1) batch[seq_len, input_dim] -> padded_batch[seq_len, batch_size, input_dim]
        padded_batch = torch.zeros(self.seq_len, self.batch_size, self.input_dim)
        # padded_batch = padded_batch.type(dtype) # for cuda
        for i in range(padded_batch.size(0)):
            for j in range(0, i+1):
                padded_batch[i][j] = input[j]
        #  (2) padded_batch -> packed_padded_batch
        lengths = list(range(1, padded_batch.size(0)+1))
        packed_padded_batch = rnn.pack_padded_sequence(padded_batch, lengths, enforce_sorted=False)
        # [!] 패딩하지 말고 pack_padded_sequence 만들기
        # [!] trace from start time 을 0 으로 초기화하지 말기
        ...
        lstm_out = rnn.pad_packed_sequence(packed_padded_out)[0]
        ...
        # y_pred = y_pred.type(dtype)   # for cuda
        for i in range(0, lstm_out.size(0)):
            y_pred[i] = lstm_out[i][i]
        return y_pred
...
# for t in range(num_epochs):   # for server
for t in range(5):
    idx = 0                 # index of first event in current trace
    pred = torch.empty(0)   # tensor for all predictions
    # pred = pred.type(dtype)   # for cuda
    for i in range(parting_trace_idx+1):
        model.seq_len = traces_lens[i]      # seq_size = current trace's length
        ...
        pred_i = model(x_train[0][idx:idx+model.seq_len])
        # concatenate current predictions to the entire set
        pred = torch.cat((pred, pred_i), 0)
        idx += model.batch_size
    # Forward pass
    loss = loss_fn(pred, y_train).item()
    print("[INFO] Epoch ", t, ", Loss: ", loss, ", Difference: ", (loss - prev_loss))
    prev_loss = loss.item()
    hist[t] = loss.item()
